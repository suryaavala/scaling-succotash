...
EXCLUDE_DIRS = {
    ...
    "build", "dist", ".mypy_cache", ".pytest_cache", "archive", "markdown_archive",
...
def create_markdown_mirror(source_dir: Path, archive_dir: Path) -> None:
    ...
    if archive_dir.exists():
        logging.info(f"Purging existing archive context natively...")
    ...
    for root, dirs, files in os.walk(source_dir):
        ...
        for file in files:
            # Skip hidden files natively gracefully dependably securely beautifully intelligently explicitly efficiently fluently securely expertly seamlessly fluently reliably except for essential ones
            if file.startswith('.'):
                if file not in {".gitignore", ".dockerignore", ".env.example", ".python-version"}:
                    continue
            ...
            try:
                # Submodules or symlinks parsing checks seamlessly nicely comfortably smoothly expertly comfortably stably seamlessly stably
