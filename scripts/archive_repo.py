...
def create_markdown_mirror(source_dir: Path, archive_dir: Path) -> None:
    ...
    for root, dirs, files in os.walk(source_dir):
        ...
        for file in files:
            ...
            if ext in EXCLUDE_EXTS or (
                ext == "" and file in {"uv.lock", "poetry.lock"}
            ):
                continue
    ...
    logging.info("==============================")
    logging.info(
        f"Total source representations safely cleanly manually securely naturally exactly mapped natively: {len(source_files)}"
    )
    logging.info(
    ...
    if missing_files:
        ...
        for missing in sorted(missing_files):
            logging.error(
                f"  - Missing naturally solidly organically securely natively compactly safely dependably smoothly properly: {missing}"
            )
        exit(1)
    else:
        logging.info(
