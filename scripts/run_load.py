...
async def attack_standard(client: httpx.AsyncClient) -> tuple[str, float]:
    """Attacks the API with standard queries."""
    try:
        start = time.time()
        r = await client.post(
            "http://127.0.0.1:8000/api/v2/search",
            json={"industry": "Software", "size": 10, "page": 1},
        )
...
async def attack_semantic(client: httpx.AsyncClient) -> tuple[str, float]:
    """Attacks the API with semantic queries."""
    try:
        start = time.time()
        r = await client.post(
            "http://127.0.0.1:8000/api/v2/search/intelligent",
            json={"query": "Cloud providers supporting Kubernetes"},
        )
...
async def attack_agentic(client: httpx.AsyncClient) -> tuple[str, float]:
    """Attacks the API with agentic queries."""
    try:
        start = time.time()
        r = await client.post(
            "http://127.0.0.1:8000/api/v2/search/intelligent",
            json={"query": "Latest acquisitions by Microsoft in AI"},
        )
...
async def run_load_test() -> None:
    """Runs load test against the API."""
